...
#ifdef MISSIONPACK
	if (gametype == GT_CTF || gametype == GT_1FCTF || gametype == GT_3WCTF)
#else
...
/*
==================
BotCTFOrders_AllFlagsNotAtBase
==================
*/
void BotCTFOrders_AllFlagsNotAtBase(bot_state_t *bs) {
...
/*
==================
BotCTFOrders_AllFlagsAtBase
==================
*/
void BotCTFOrders_AllFlagsAtBase(bot_state_t *bs) {
	int numteammates, defenders, attackers, i;
	int teammates[MAX_CLIENTS] = {0};
	char name[MAX_NETNAME];
...
void BotCTFOrders(bot_state_t *bs) {
	...
	else flagstatus = bs->blueflagstatus * 2 + bs->redflagstatus * 2 + bs->greenflagstatus;
	//
	if (BotTeam)(bs) == TEAM_BLUE) flagstatus = bs->blueflagstatus * 2 + bs->redflagstatus * 2 + bs->greenflagstatus;
	else flagstatus = bs->redflagstatus * 2 + bs->blueflagstatus * 2 + bs->greenflagstatus;
	//
...
void BotTeamAI(bot_state_t *bs) {
	...
	switch(gametype) {
		...
		case GT_3WCTF:
		{
			// if the enemy teams lead and time limit has expired to 70%, choose aggressive strategy
			if (bs->ownteamscore < bs->enemyteamscore && level.time - level.startTime > (g_timelimit.integer * 60000) * 0.7f) {
				bs->ctfstrategy = CTFS_AGRESSIVE;
			} else {
				// if there were no flag captures the last 4 minutes
