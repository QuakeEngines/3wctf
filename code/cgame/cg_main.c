...
static cvarTable_t cgameCvarTable[] = {
	...
	{ &cg_greenTeamName, "g_greenteam", DEFAULT_GREENTEAM_NAME, CVAR_ARCHIVE | CVAR_SYSTEMINFO, RANGE_ALL },
...
void CG_RegisterCvars( void ) {
	...
	greenTeamNameModificationCount = cg_greenTeamName.modificationCount;
#endif
}
...
static void CG_RegisterSounds( void ) {
	...
	if ( cgs.gametype >= GT_TEAM || cg_buildScript.integer ) {
		...
		// coltonquake3: Re-enabled capture award sound to use flagcap sounds from pak0.pk3 & the green team resource pk3.
		// FIXME: Is it possible to have multiple sounds play over each other with such sound command?
		// Will remove fixme stuff if possible.
		cgs.media.captureAwardSound = trap_S_RegisterSound( "sound/teamplay/flagcap_blu.wav", qtrue );
		cgs.media.captureAwardSound = trap_S_RegisterSound( "sound/teamplay/flagcap_red.wav", qtrue );
		cgs.media.captureAwardSound = trap_S_RegisterSound( "sound/teamplay/flagcap_grn.wav", qtrue );
		cgs.media.redLeadsSound = trap_S_RegisterSound( "sound/feedback/redleads.wav", qtrue );
		cgs.media.blueLeadsSound = trap_S_RegisterSound( "sound/feedback/blueleads.wav", qtrue );
		cgs.media.greenLeadsSound = trap_S_RegisterSound( "sound/feedback/greenleads.wav", qtrue );
		...
		if ( cgs.gametype == GT_CTF || cgs.gametype == GT_3WCTF || cg_buildScript.integer ) {
		...
		if ( cgs.gametype == GT_1FCTF || cg_buildScript.integer ) {
			// FIXME: get a replacement for this sound? coltonquake3: Edited blue flag returned to get this sound
		...
		if ( cgs.gametype == GT_1FCTF || cgs.gametype == GT_CTF || cgs.gametype == GT_3WCTF || cg_buildScript.integer ) {
...
static void CG_RegisterGraphics( void ) {
	...
	cgs.media.greenProxMine = trap_R_RegisterModel( "models/weaphits/proxmineg.md3" );
#endif
	...
	if ( cgs.gametype == GT_HARVESTER || cg_buildScript.integer ) {
		...
		cgs.media.greenCubeModel = trap_R_RegisterModel( "models/powerups/orb/g_orb.md3" );
		cgs.media.redCubeIcon = trap_R_RegisterShader( "icons/skull_red" );
		cgs.media.blueCubeIcon = trap_R_RegisterShader( "icons/skull_blue" );
		cgs.media.greenCubeIcon = trap_R_RegisterShader( "icons/skull_green" );
	}

	if ( cgs.gametype == GT_CTF || cgs.gametype == GT_3WCTF || cgs.gametype == GT_1FCTF || cgs.gametype == GT_HARVESTER || cg_buildScript.integer ) {
#else
	if ( cgs.gametype == GT_CTF || cgs.gametype == GT_3WCTF || cg_buildScript.integer ) {
#endif
		...
		cgs.media.greenFlagShader[0] = trap_R_RegisterShaderNoMip( "icons/iconf_grn1" );
		cgs.media.greenFlagShader[1] = trap_R_RegisterShaderNoMip( "icons/iconf_grn2" );
		cgs.media.greenFlagShader[2] = trap_R_RegisterShaderNoMip( "icons/iconf_grn3" );
#ifdef MISSIONPACK
		...
		CG_RegisterSkin( "models/flag2/green.skin", &cgs.media.greenFlagFlapSkin, qfalse );
		CG_RegisterSkin( "models/flag2/white.skin", &cgs.media.neutralFlagFlapSkin, qfalse );

		cgs.media.redFlagBaseModel = trap_R_RegisterModel( "models/mapobjects/flagbase/red_base.md3" );
		cgs.media.blueFlagBaseModel = trap_R_RegisterModel( "models/mapobjects/flagbase/blue_base.md3" );
		cgs.media.greenFlagBaseModel = trap_R_RegisterModel( "models/mapobjects/flagbase/grn_base.md3" );
	...
	if ( cgs.gametype == GT_1FCTF || cg_buildScript.integer ) {
		...
		cgs.media.flagShader[3] = trap_R_RegisterShaderNoMip( "icons/iconf_grn2" );
	...
	if ( cgs.gametype == GT_HARVESTER || cg_buildScript.integer ) {
		...
		CG_RegisterSkin( "models/powerups/harvester/green.skin", &cgs.media.harvesterGreenSkin, qfalse );
		cgs.media.harvesterNeutralModel = trap_R_RegisterModel( "models/powerups/obelisk/obelisk.md3" );
#endif
#ifdef MISSIONPACK
		cgs.media.redKamikazeShader = trap_R_RegisterShader( "models/weaphits/kamikred" );
		cgs.media.blueKamikazeShader = trap_R_RegisterShader( "models/weaphits/kamikblu" );
		cgs.media.greenKamikazeShader = trap_R_RegisterShader( "models/weaphits/kamikgrn" );
#endif

	if ( cgs.gametype >= GT_TEAM || cg_buildScript.integer ) {
		cgs.media.friendShader = trap_R_RegisterShader( "sprites/foe" );
		cgs.media.redQuadShader = trap_R_RegisterShader( "powerups/redflag" );
		cgs.media.greenQuadShader = trap_R_RegisterShader( "powerups/greenflag" );
...
static int CG_FeederCount(float feederID) {
	...
	if (feederID == FEEDER_REDTEAM_LIST) {
		...
		for (i = 0; i < cg.numScores; i++) {
			if (cg.scores[i].team == TEAM_GREEN) {
				count++;
			}
		}
...
static void CG_FeederSelection(float feederID, int index) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		int i, count;
		int team = (feederID == FEEDER_REDTEAM_LIST) ? TEAM_RED : TEAM_BLUE: TEAM_GREEN;
		count = 0;
		for (i = 0; i < cg.numScores; i++) {
			if (cg.scores[i].team == team) {
				if (index == count) {
...
static int CG_OwnerDrawWidth(int ownerDraw, float scale) {
	switch (ownerDraw) {
	  ...
	  case CG_BLUE_NAME:
			...
			break;
	  case CG_GREEN_NAME:
			return CG_Text_Width(cg_greenTeamName.string, scale, 0);
			break;	
