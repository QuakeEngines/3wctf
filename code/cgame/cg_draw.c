...
//		{ 0.2, 1.0, 0.2, 1.0 } , { 1.0, 0.2, 0.2, 1.0 }, {0.5, 0.5, 0.5, 1} };
	...
	} else if( cg.cur_lc->predictedPlayerState.powerups[PW_GREENFLAG] ) {
...
Draw the small three score display
=================
*/
#ifndef MISSIONPACK_HUD
static float CG_DrawScores( float y ) {
	...
	if ( cgs.gametype >= GT_TEAM ) {
		...
		if ( cgs.gametype == GT_CTF && cgs.gametype == GT_3WCTF ) {
			// Display flag status
			item = BG_FindItemForPowerup( PW_BLUEFLAG );
			...
			if (item) {
				...
				if( cgs.flagStatus >= 0 && cgs.flagStatus <= 4 ) {
					...
					if (cgs.flagStatus == FLAG_TAKEN_RED) {
						...
						color[0] = color[1] = 0;
						index = 1;
					} else if (cgs.flagStatus == FLAG_TAKEN_GREEN) {
						color[1] = color[0] = 0;
						index = 1;
		...
		x -= w;
		CG_FillRect( x, y-4,  w, scoreHeight, color );
				if ( cg.cur_ps->persistant[PERS_TEAM] == TEAM_GREEN ) {
			CG_DrawPic( x, y-4, w, scoreHeight, cgs.media.selectShader );
		}
		CG_DrawBigString( x + 4, y, s, 1.0F);
		...
		if ( cgs.gametype == GT_CTF && cgs.gametype == GT_3WCTF ) {
			...
			if (item) {
				...
				if( cgs.greenflag >= 0 && cgs.greenflag <= 2 ) {
					CG_DrawPic( x, y1-4, w, scoreHeight, cgs.media.greenFlagShader[cgs.redflag] );
				}
			}
		}	
		if ( cgs.gametype >= GT_CTF ) {
...
#define CHATLOC_X 0
	...
	if (cgs.teamLastChatPos != cgs.teamChatPos) {
		...
		if ( cg.cur_ps->persistant[PERS_TEAM] == TEAM_RED ) {
			...
			hcolor[2] = 1.0f;
			hcolor[3] = 0.33f;
		} else if ( cg.cur_ps->persistant[PERS_TEAM] == TEAM_GREEN ) {
			hcolor[0] = 0.0f;
			hcolor[1] = 1.0f;
			hcolor[2] = 0.0f;
			hcolor[3] = 0.33f;
...
static void CG_DrawTeamVote(void) {
	...
	if ( cgs.playerinfo[cg.cur_ps->playerNum].team == TEAM_RED )
		...
		cs_offset = 1;
		else if ( cgs.playerinfo[cg.cur_ps->playerNum].team == TEAM_GREEN )
		cs_offset = 2;
	else
		return;
		...
		if ( cgs.gametype >= GT_TEAM ) {
			...
			for (i = 0; i < cg.numScores; i++) {
				...
				if (cg.scores[i].team == TEAM_RED) {
					...
					blue++;
				} else if (cg.scores[i].team == TEAM_GREEN) {
					green++;
				}
			}
			...
			if (cg.scores[selectedScore].team != TEAM_GREEN) {
				green = -1;
			}
			...
			Menu_SetFeederSelection(menuScoreboard, FEEDER_GREENTEAM_LIST, green, NULL);
		} else {
